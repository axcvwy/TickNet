      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
          <div class="flex items-center gap-3">
            <h1
              class="text-xl font-bold bg-gradient-to-r from-purple-600 to-orange-500 bg-clip-text text-transparent"
            >
        ...
        <div class="max-w-6xl mx-auto">
          ...
          <div v-if="currentTab === 'overview'" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <!-- Stat Card 1 -->
              <div
                class="bg-white p-6 rounded-xl shadow-sm border border-orange-100 relative overflow-hidden group"
              >
              ...
              <!-- Stat Card 2 -->
              <div
                class="bg-white p-6 rounded-xl shadow-sm border border-purple-100 relative overflow-hidden group"
              >
              ...
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="spec in spectacles" :key="spec.id">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {{ spec.dureeMinutes }} min
                    </td>
                    <td
                      class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3"
                    >
                      <button
                        @click="openModal('spectacle', spec)"
                        class="text-purple-600 hover:text-purple-900"
                      >
                        Modifier
                      </button>
                      <button
                        @click="handleDeleteSpectacle(spec)"
              ...
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="salle in salles" :key="salle.id">
                    ...
                    >
                      <button
                        @click="openModal('salle', salle)"
                        class="text-purple-600 hover:text-purple-900"
                      >
                        Modifier
                      </button>
                      <button
                        @click="handleDeleteSalle(salle)"
                        class="text-red-600 hover:text-red-900"
                      >
                        Supprimer
                      </button>
              ...
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="sess in sessions" :key="sess.id">
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                      {{ sess.spectacle.titre }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {{ sess.salle.nom }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {{ formatDate(sess.dateHeure) }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {{ formatCurrency(sess.prix) }}
                    </td>
                    <td
                      class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3"
                    >
                      <button
                        @click="openModal('session', sess)"
                        class="text-purple-600 hover:text-purple-900"
                      >
                        Modifier
                      </button>
                      <button
                        @click="handleDeleteSession(sess)"
                        class="text-red-600 hover:text-red-900"
                      >
                        Supprimer
                      </button>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="res in reservations" :key="res.id" class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      ...
                      >
                        {{ res.statut === 'CANCELLED' ? 'Annulée' : 'Confirmée' }}
                      </span>
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <button
                        ...
                        @click="handleCancelReservation(res)"
                        class="text-red-600 hover:text-red-900"
                      >
                        Annuler
                      </button>
                      <button v-else disabled class="text-gray-400">Annulée</button>
                    </td>
                  </tr>
                </tbody>
              </table>
        ...
        <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
          ...
          <template v-if="modalType === 'spectacle'">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  v-model="formSpec.genre"
                  required
                  class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 sm:text-sm border p-2 bg-white"
                >
          ...
          <template v-else-if="modalType === 'salle'">
            ...
            <div>
              ...
              <input
                v-model="formSalle.localisation"
                required
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 sm:text-sm border p-2"
              />
            </div>
          ...
          <template v-else>
            <div>
              ...
              <select
                v-model="formSession.spectacleId"
                required
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 sm:text-sm border p-2"
              >
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700"
                  >Capacité de la session</label
                >
                <input
                  type="number"
                  v-model="formSession.capacite"
                  class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 sm:text-sm border p-2"
                />
...
function openModal(type: 'spectacle' | 'salle' | 'session', item?: any) {
  ...
  if (type === 'spectacle') {
    Object.assign(
      formSpec,
      item || { titre: '', description: '', imageUrl: '', genre: '', dureeMinutes: 120 },
    )
  } else if (type === 'salle') {
...
async function handleCancelReservation(res: Reservation) {
  ...
  try {
    ...
    await fetchData() // recharge la liste + stats
  } catch (e) {
